            ...
            <div class="header-middle-wrap">
                <div class="brand-area">
                    <a class="brand-logo" href="{{ route('front') }}">
                        <img class="brand-image" src="{{ asset(IMG_LOGO_PATH . $allsettings['main_logo']) }}" alt="{{ $allsettings['app_title'] }}" />
                    </a>
                </div>
                ...
                <div class="search-area">
                    <form action="{{ route('category.product') }}" method="get">
                        <div class="search-wrap">
                            <input type="text" class="form-control !border-none !outline-none !ring-0 placeholder:text-2xl placeholder:ps-5" id="search" name="search" placeholder="{{ __('Search Here') }}" />
                            <button type="submit" class="search-btn"><i class="flaticon-search mt-2 inline-block"></i></button>
                        </div>
                    </form>
                </div>
                    ...
                    <div class="header-top-right flex items-center gap-5">
                        <div class="switcher-lang-currency flex items-center gap-5">
                            <div class="lang-switcher">
                                ...
                                @endif
                                <ul class="{{ getLanguage('en')->status != 1 || getLanguage('fr')->status != 1 ? null : 'lang-list' }}">
                                    @if (app()->getLocale() == 'en')
                        ...
                        <div class="account-switcher text-center">
                            <span class="flag">
                                <img src="{{ file_exists(AdminProfilePicture() . Auth::user()->image) ? (isset(Auth::user()->image) ? asset(AdminProfilePicture() . Auth::user()->image) : Avatar::create(Auth::user()->name)->toBase64()) : Auth::user()->image }}" alt="{{ $allsettings['app_title'] }}">
                            </span>
                            ...
                            <ul class="account-list">
                                <li class="single-lang">
                                    <a class="lang-text" href="{{ route('user.profile.edit') }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-8 h-8 mb-2.5">
