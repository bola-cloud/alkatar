                ...
                <div class="single-grid-product">
                    ...
                    <div class="product-info">
                        <div class="product-name-part">
                            <h3 class="product-name"><a class="product-link" href="javascript:void(0)">{{
                                    langConverter($item->name, $item->options->name_ar)
                                    }}</a></h3>
                            <div class="cart-quantity input-group">
                                <div class="increase-btn dec qtybutton btn qty_decrease" data-id="{{ $item->rowId }}">-
                                </div>
                                <input class="qty-input cart-plus-minus-box qty_value" type="text" name="qtybutton"
                                    id="qty_value" value="{{ $item->qty }}" readonly />
                                <div class="increase-btn inc qtybutton btn qty_increase" data-id="{{ $item->rowId }}">+
                                </div>
                            </div>
                            <button class="cart-remove-btn deleteItem" data-id="{{ $item->rowId }}">{{ __('Remove')
                                }}</button>
                        </div>
