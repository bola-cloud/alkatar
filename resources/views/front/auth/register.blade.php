...
@section('title', __('new_design.auth.register_title') . ' | ' . 'بن القطار')
    ...
    <div class="bg-[#F8F9F8]/95 backdrop-blur-sm rounded-3xl shadow-2xl p-8 lg:p-12 w-full max-w-[680px] {{ $isRtl ? 'lg:ml-auto' : 'lg:mr-auto' }}">
        ...
        <div class="mb-6 text-center {{ $isRtl ? 'lg:text-right' : 'lg:text-left' }}">
            <h1 class="text-3xl lg:text-[34px] font-black text-[#1A4231] mb-2 leading-tight">
                {{ __('new_design.auth.register_title') }}
            </h1>
            <p class="text-slate-600 text-sm font-semibold mt-1">
                {{ __('new_design.auth.register_subtitle') }}
            </p>
        </div>
        ...
        <!-- Form -->
        <form action="{{ route('user.sign.up.post') }}" method="POST" class="space-y-4">
            @csrf

            <!-- Full Name -->
            <div class="{{ $isRtl ? 'text-right' : 'text-left' }}">
                <label class="block text-[13px] font-bold text-gray-700 mb-2 {{ $isRtl ? 'pr-1' : 'pl-1' }}">
                    {{ __('new_design.auth.full_name') }}
                </label>
                <div class="relative">
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="{{ __('new_design.auth.full_name') }}" required
                           class="w-full px-5 py-3 rounded-xl border @error('name') border-red-500 @else border-gray-300 @enderror bg-transparent outline-none focus:border-[#1A4231] focus:ring-1 focus:ring-[#1A4231] transition-all text-sm font-semibold text-gray-800">
                    @error('name')
            ...
            <button type="submit" class="w-full bg-[#1A4231] text-white py-3.5 rounded-xl font-bold text-[15px] shadow-lg hover:bg-[#133224] transition-all flex items-center justify-center gap-3 mt-4">
                {{ __('new_design.auth.register_btn') }}
        ...
        <p class="mt-8 text-center text-[12px] font-semibold text-gray-500">
            {{ __('new_design.auth.have_account') }} 
            <a href="{{ route('login') }}" class="text-[#1A4231] font-bold hover:underline">
                {{ __('new_design.auth.login_link') }}
            </a>
        </p>
    </div>
</div>
