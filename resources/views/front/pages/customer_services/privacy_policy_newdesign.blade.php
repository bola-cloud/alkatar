...
@section('title', isset($title) ? $title : __('Privacy Policy'))
@section('description', isset($description) ? $description : '')
@section('keywords', isset($keywords) ? $keywords : '')
@section('content')

    @php
        $bannerTitle = $title ?? __('Privacy Policy');
        $locale = session('HTML_LANG', session('APP_LOCALE', app()->getLocale() ?? 'en'));
        $isAr = in_array($locale, ['ar', 'fr']);
        $content = CutomerServiceContent('privacy_policy');
    ...
    <div class="container py-5">
        <div class="row">
            ...
            {{-- <div class="col-lg-4 d-none d-lg-block text-center">
                <img src="{{ asset('new-design/images/privacy.png') }}" alt="privacy" class="img-fluid"
