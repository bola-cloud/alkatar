...
class ProductController extends Controller
{
    ...
    {
        $product = Product::where('en_Product_Slug', $slug)->with('category')->where('Status', 1)->where('Quantity', '>', 0)->firstOrFail();
        if (!empty($product)) {
            $cat_id = $product->category?->id;

            $data['similar_product'] = Product::with('brand', 'category', 'colors', 'sizes', 'product_tags')->where('Status', 1)->where('Quantity', '>', 0)
    ...
    public function allProduct()
    {
        ...
        $products = Product::with([
            ...
            'product_tags'
        ])->where('Status', 1)->where('Quantity', '>', 0)->orderByRaw('TRIM(fr_Product_Name) COLLATE utf8mb4_unicode_ci ASC')->paginate(9);
        $data['products'] = $products;
    ...
    public function productSorting(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($value == 'Categories') {
                $filters = Product::where('Status', 1)->where('Quantity', '>', 0)->where('Category_Id', '!=', null)->orderByRaw('TRIM(fr_Product_Name) COLLATE utf8mb4_unicode_ci ASC')->get();
                if ($filters) {
                    return view('front.pages.product.filter_product', compact('filters'));
                }
            } elseif ($value == 'Brands') {
                $filters = Product::where('Status', 1)->where('Quantity', '>', 0)->where('Brand_Id', '!=', null)->orderByRaw('TRIM(fr_Product_Name) COLLATE utf8mb4_unicode_ci ASC')->get();
                if ($filters) {
                    return view('front.pages.product.filter_product', compact('filters'));
                }
            } elseif ($value == 'Products') {
                $filters = Product::where('Status', 1)->where('Quantity', '>', 0)->orderByRaw('TRIM(fr_Product_Name) COLLATE utf8mb4_unicode_ci ASC')->get();
                if ($filters) {
                    return view('front.pages.product.filter_product', compact('filters'));
                }
            }
    ...
    public function productFiltering(Request $request)
    {
        ...
        if ($request->ajax()) {
            if ($request->checkCat) {
                $filters = Product::where('Status', 1)->where('Quantity', '>', 0)->with([
                    'category',
                    'additions' => function ($query) {
                        $query->where('status', 1);
                    }
                ...
                $filters = Product::where('Status', 1)->where('Quantity', '>', 0)->where('en_Product_Name', 'LIKE', "%{$request->search}%")->orderByRaw('TRIM(fr_Product_Name) COLLATE utf8mb4_unicode_ci ASC')->get();
            } elseif ($request->min && $request->max) {
                $filters = Product::where('Status', 1)->where('Quantity', '>', 0)->whereBetween('Discount_Price', [$request->min, $request->max])->orderByRaw('TRIM(fr_Product_Name) COLLATE utf8mb4_unicode_ci ASC')->get();
    ...
    public function productFilteringLeftSide(Request $request)
    {
        if ($request->ajax()) {
            if ($request->checkCat) {
                $filters = Product::where('Status', 1)->where('Quantity', '>', 0)->with('category')->whereHas('category', function ($query) use ($request) {
                    $query->whereIn('en_Category_Name', $request->checkCat);
                })->orderByRaw('TRIM(fr_Product_Name) COLLATE utf8mb4_unicode_ci ASC')->get();
            } elseif ($request->checkBrand) {
                $filters = Product::where('Status', 1)->where('Quantity', '>', 0)->with('brand')->whereHas('brand', function ($query) use ($request) {
                    $query->whereIn('en_BrandName', $request->checkBrand);
                })->orderByRaw('TRIM(fr_Product_Name) COLLATE utf8mb4_unicode_ci ASC')->get();
            } elseif ($request->checkColor) {
                $filters = Product::where('Status', 1)->where('Quantity', '>', 0)->with('colors')->whereHas('colors', function ($query) use ($request) {
                    $query->whereIn('Name', $request->checkColor);
                })->orderByRaw('TRIM(fr_Product_Name) COLLATE utf8mb4_unicode_ci ASC')->get();
            } elseif ($request->checkSize) {
                $filters = Product::where('Status', 1)->where('Quantity', '>', 0)->with('sizes')->whereHas('sizes', function ($query) use ($request) {
                    $query->whereIn('Size', $request->checkSize);
                })->orderByRaw('TRIM(fr_Product_Name) COLLATE utf8mb4_unicode_ci ASC')->get();
            } elseif ($request->search) {
                $filters = Product::where('Status', 1)->where('Quantity', '>', 0)->where('en_Product_Name', 'LIKE', "%{$request->search}%")->orderByRaw('TRIM(fr_Product_Name) COLLATE utf8mb4_unicode_ci ASC')->get();
    ...
    public function CategoryWiseProductLeft($id)
    {
        ...
        $products = Product::with([
            ...
            'product_tags'
        ])->where('Status', 1)->where('Quantity', '>', 0)->where('Category_Id', $id)->orderByRaw('TRIM(fr_Product_Name) COLLATE utf8mb4_unicode_ci ASC')->paginate(9);
        $data['products'] = $products;
        $seo = SeoSetting::where('slug', 'all-products')->first();
        $data['title'] = $seo->title;
        $data['description'] = $seo->description;
    ...
    public function BrandWiseProduct($id)
    {
        ...
        $data['category'] = Category::where('Status', 1)->get();
        $data['brands'] = Brand::with('products')->get();
        $products = Product::with('brand', 'category', 'colors', 'sizes', 'product_tags')->where('Status', 1)->where('Quantity', '>', 0)->where('Brand_Id', $id)->orderByRaw('TRIM(fr_Product_Name) COLLATE utf8mb4_unicode_ci ASC')->paginate(9);
        $data['products'] = $products;
        $seo = SeoSetting::where('slug', 'all-products')->first();
        $data['title'] = $seo->title;
        $data['description'] = $seo->description;
